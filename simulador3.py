...
def realizar_instrucao(instrucao):
    ...
    if((instrucao.ins)=='M'):
        ...
        Tempo=Tempo+1 
        return 1
        ## Ver melhor esta parte(x)
#--------------------Realizar as instruçoes------------------------

def SJF_RunningState():
    global Prontos
    global RunningState
    aux=(0,-1)
    for i in range(len(Prontos)):
        if(aux[1]>Prontos[i].tamanho or aux[1]==-1):
            x=i
            y=Prontos[i].tamanho
            aux=(x,y)
    if(RunningState!=None):
...
def processos_concorrentes3(pid):#OK
    global RunningState
    global Bloqueados
    global Memory
    global Tempo
